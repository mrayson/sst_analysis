...
def parse_input():
    ...
    parser.add_option("-c", "--coordinates", dest="c",
                      ...
                      default='110.,135.,-30.,-5')
    parser.add_option("-n", "--name", dest="name",
                      help="box name", default='NWS')    
...
# download data
#
def download_hw(times, rpath, dpath=None, box=None, remove_raw=False):
    ...
    for time in times:
        ...
        fdir = '/pub/himawari/L3/SST/'+VER+'_nc4_normal_std/'+YYYYMM+'/'+DD+'/'
        #fname_pref = YYYYMMDDhhmm+'00-JAXA-L2P_GHRSST-SSTskin-H08_AHI_NRT-'+ \
        #fname_pref = YYYYMMDDhhmm+'00-JAXA-L3C_GHRSST-SSTskin-H08_AHI-'+ \
        ...
        #if os.path.isfile(dpath+fname_out):
        #    print('File {} already exists... skipping'.format(fname_out))
        #    continue
        ...
        try:
            ...
            for fname in files:
                #
                fileraw = rpath+fname #fname_pref
                sstfiles.append(fileraw)
                #
                #if not os.path.isfile(fileraw):
                print('Downloading from jaxa ftp: '+fname)
                ...
                if remove_raw:
                    print('Removing raw file: {}'.format(fileraw))
                    os.remove(fileraw)
        except:
...
def process_raw_data(fileraw,box,fname_out, \
                     variables=['sea_surface_temperature','wind_speed','solar_zenith_angle','l2p_flags','quality_level']):
    """ Open the raw file and zoom into box
    """
    try:
        ds = xr.open_dataset(fileraw).sel(lon=box['lon'],lat=box['lat'])
    except:
        print('Xarray failed to open file {}...'.format(fileraw))
        return
    #print(ds)
    ...
    try:
        ...
        print('failed to writefile {}...'.format(dpath+fname_out))
        return
...
if __name__ == "__main__":
    ...
    #rpath = 'cloudstor/Data/himawari_sst/raw/'
